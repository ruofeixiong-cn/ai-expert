...
async def retrieve(tenant_id: UUID, expert_id: UUID, question: str) -> list[Hit]:
    ...
    for idx, score in ranked:
        ...
        effective = score * (row.confidence if row.confidence is not None else 1.0)
        if effective < RERANK_MIN_SCORE:
            continue
        hits.append(Hit(chunk_id=row.id, content=row.content, source=row.source, score=effective))
        if len(hits) >= FINAL_TOP_K:
            break

    log.info(
        "召回 expert=%s 候选=%d 通过闸门=%d 最高分=%.4f",
        expert_id, len(rows), len(hits), hits[0].score if hits else 0.0,
    )
    return hits
