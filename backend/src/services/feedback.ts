 ...
 * 疑似盲区的置信度阈值。
 *
 * 实测（真实 rerank，同一个专家、同一批切片）：
 *   「定投的手续费怎么算」  0.8375  ← 库里讲过
 *   「港股打新要准备多少现金」0.1163  ← 库里没讲过
 *   M3 另一组：相关 0.2149~0.2956，无关 0.0059，入口闸门 0.05
 *
 * 0.15 落在两簇之间，但样本很少 —— 这仍是个**占位值**，M6 用黄金问答集校准。
 *
 * 因为盲区判定不落库（见 specs/005-m4-feedback/spec.md §5.3），
 * 改这个常量当天，全部历史自动按新口径重算。
 */
